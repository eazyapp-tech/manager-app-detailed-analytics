...
## DA-08 Inventory / Occupancy — node map (added 2026-08-08)

File key `KgBQXiT7r7oGrcqZHCWxyU`. Build target: section **"Occupancy insight - done"** = `10764:115945`. The older draft (superseded, but the source five recovered features came from) is section **"OCCUPANCY"** = `6413:90964`.

| What | Node |
|---|---|
| The screen frame | `10611:113044` |
| Overview Snapshot | `10611:113136` |
| View toggle (screen level) | `10611:113131` |
| Occupancy Status | `10611:113248` |
| Vacant Room Status | `10611:113508` |
| Upcoming Vacancy | `10611:113551` |
| Occupancy Trend | `10611:113641` |
| Where is my property losing money? | `10611:113370` |
| Property Wise Occupancy (layer name still "Property Expense") | `10905:110782` |
| Empty states set | `10764:116537` |
| Orphan tile row outside the phone | `11114:111035` |
| Staged Agreements-ending redesign (post-sheet, not yet placed) | `12580:116669` |

Traps specific to this section: three hidden dead delta chips sit under Overview tiles (`10611:113799`, `113835`, `113847`); the trend's fifth "Move-out" column is clipped invisible but present (`10611:113697`); FlexiPe and Report chips are clipped outside the 399px filter row, in the scrollable row, not deleted.
...
Three live repos, each its own checkout. The backend answers what the system records; the app repos answer what a manager actually sees, which is a different question and the one this suite is usually asking.

| Repo | Where | Remote |
|---|---|---|
| **Backend** | `~/rentok-backend` | `eazyapp-tech/rentok-backend` |
| **Manager app (Flutter)** | `~/RentOk_Manager_App/rentokmanagerflutter` | `eazyapp-tech/rentokmanagerflutter` |
| **Manager web app** | `~/RM Manager WebApp/rentok-manager-web` | `eazyapp-tech/rentok-manager-web` |

The web app has feature worktrees beside it (`-billing`, `-tasks`, `-food`), all the same remote. Use the plain `rentok-manager-web` unless a feature branch is the point.

⚠ **`~/RentOk_FE_BE` is stale and must not be used.** Its Flutter copy last moved 2026-04-29 against the live repo's 2026-07-28, three months of drift, and it is the kind of source that reads exactly like a current one. It was cited in an earlier version of this file; that was wrong.

**When to reach for the app repos:** what a list screen renders, which filter chips the UI exposes, what a tap does today, what a card's empty state actually says. On Tenants, several "does this list offer that filter" questions were answered from backend request handling alone. That tells you what the system will accept, not what a manager can reach.
