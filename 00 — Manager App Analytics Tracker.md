...
|---|---|---|---|---|---|
| 1 | Financial | **Dues** | ✅ Closed — v11, restructured to the current standard | 3 hardened docs (reconciled), now superseded | [[DA-01 Dues — Handoff Sheet]] · [[DA-01 Cover Note — Nimit]] · [[DA-01 Cover Note — Design]] |
...
**Collection backfill (DA-02) — bring the second sheet up to the current standard**, using the checklist above and Dues v11 as the worked example of every item on it.

1. Read [[DA-01 Dues — Handoff Sheet]] (v11) first, not as a source of Collection content, as the reference for what "done" looks like now: the spine, the Others-overflow pattern, the cross-tab drill rule, the build-status framing.
2. Run production sizing before drafting, not after, Dues found the gauge and ageing chart's real shape (99.5% overdue, 92% in the oldest bucket) this way and it reshaped two cards.
3. Collection's code-critique section is its single biggest item, budget for it.
4. Watch the self-added-tenant permission question, it recurs on Collection and is still genuinely open (see Open questions below).
5. Sibling-check against both Dues and Tenants when done, Dues because they share a permission and a tab, Tenants because it's still the density/voice exemplar.
