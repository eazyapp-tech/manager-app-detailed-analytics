...
### Collection backfill checklist

All eight items below were also applied to Dues during its 8 August backfill, confirmed working there. Collection is the only screen left carrying them.

1. **Restructure to one spine, numbered end to end.** Collection numbers its eight cards but leaves everything around them unnumbered. It cannot carry a reading map until it does.
2. **One name per section across the suite.** The Restricted state's section name must match the others.
3. **Separate the design-only sections** so a backend reader can be told to skip them.
4. **Query production for every number the sheet specifies**, appendix the figures.
5. **Make share-of-total a requirement on every breakdown row**, not a maybe.
6. **Write the cover note(s)** — which sections are the reader's, which decisions are pre-answered, where the superseded sources sit.
7. **Strip every code critique and code reference**, restate as outcome plus test. Collection's *"What is already broken in the code this will be built from"* section is the largest single case in the suite, this is the biggest item in its backfill.
8. **Three cross-cutting sections may be missing**: loading and failure, the unfinished-period comparison, and the rule that a drill destination names what it is showing. Check against what Dues v11 now has.
...
**Collection backfill (DA-02).** Use the checklist above, and use Dues (v12) as the worked example of every item on it: spine, cell vocabulary, Others-overflow pattern, cross-tab drills, GitHub navigation, the reader test.

1. Read [[DA-01 Dues — Handoff Sheet]] first, not for Collection content, but as the picture of what finished looks like now.
2. **Run production sizing before drafting, not after.** On Dues it reshaped two cards; on Expense it killed three specced sections.
3. **The code-critique section is the single biggest item in this backfill.** Every line becomes an outcome plus a test.
4. **Audit the Due Date / Paid Date toggle against all three failure modes** (collapses · goes dead or duplicates · silently changes meaning). The third one announces nothing and only a written-out definition per side catches it. Collection is the screen this trap was found on.
5. **Decide the forward setting deliberately.** Dues got Coming up because bills carry future dates. Collection is payments, which are past events, so it may genuinely not need one. Argue it rather than copying either way.
6. **Sibling-check against Dues and Tenants both.** Dues because they share a tab and a permission; Tenants because it is still the density and voice exemplar.
7. Sweep every deliverable in the folder at close-out, cover notes included, not the sheet alone.

### After Collection

**Old Tenants** (screen 6; its pre-flight below still stands), then the **Expense + Inventory uplift** in one lighter session (arrival-filter column, window-travel, contracts, dash sweep).
