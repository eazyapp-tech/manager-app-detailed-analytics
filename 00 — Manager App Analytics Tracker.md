...
All eight items applied at v13, same as Dues' 8 August backfill. The queue now holds only the Expense + Inventory uplift.
...
**Old Tenants (screen 6)**, the second sub-tab under People. Its pre-flight below stands. After it: the **Expense + Inventory uplift** in one lighter session (arrival-filter column, window-travel, contracts, dash sweep), then **Bookings** (inherits the booking definitions, and the "awaiting confirmation" number disappearing under a filter needs a ruling), then **Complaints**, then the new **Leads** screen.
