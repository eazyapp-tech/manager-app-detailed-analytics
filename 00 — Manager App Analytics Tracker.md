...
| 8 | Complaints | **Complaints** | ⬜ Not started | Unknown | Empty-state copy leaking into Dues widgets originates from this module. **Reprioritized ahead of Old Tenants and Bookings at the owner's request, 2026-08-09** |
...
**Complaints (screen 8)**, its own tab, not under People. **Reprioritized ahead of Old Tenants and Bookings at the owner's request (2026-08-09)**; those two remain not started and keep their place in the order after this one, followed by Leads.

Known going in, so Phase 0 doesn't have to rediscover it:

- The design file's own tab row calls this tab "Issues". The settled name is **Complaints** — keep it, the redraw is design's problem not this doc's.
- The developer spreadsheet has a tab for this screen, also named "Issues", not Complaints.
- Tenants found an undocumented "Advanced Insights" second-screen concept under its own name and noted **Issues carries the same thing**. Check for it and rule deferred, dead or forgotten, same as any hidden layer.
- **This is the module the leaked Complaints/Maintenance empty-state copy on Dues (and likely other screens) originated from.** Worth double-checking its own copy is actually right, since other screens borrowed from it on the assumption that it was.
- Needs the **today number / window number split** from Tenants (open-complaint counts and complaints-raised-this-month are the same two kinds side by side) and the **healthy-vs-empty distinction** from Inventory (zero open complaints is good news, not a gap) — both already flagged as likely needed here before this session.
- "When a number is red" (Tenants' rule) applies here **in reverse**: most other screens redden an obligation missed, Complaints inverts it, an open complaint aging past its own SLA is the red case.
- Run the new frame-dismissal re-check rule the moment this sheet writes any claim about what its design file does or doesn't contain.
