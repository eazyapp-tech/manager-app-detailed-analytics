...
|---|---|---|---|---|---|
| 1 | Financial | **Dues** | ✅ Closed, v12. The current reference sheet | 3 hardened docs (reconciled), now superseded | [[DA-01 Dues — Handoff Sheet]] · [[DA-01 Cover Note — Nimit]] · [[DA-01 Cover Note — Design]] |
| 2 | Financial | **Collection** | ✅ **Closed — v13, restructured and production-sized** | 3 hardened docs (superseded) + archived Drill Map + dev sheet | [[DA-02 Collection — Handoff Sheet]] + [[DA-02 Cover Note — Nimit]] + [[DA-02 Cover Note — Design]]. Template spine, tap matrix with drawer-verified verdicts, 18 production measurements, Coming up added (Due Date view only), code-critique section replaced by outcome-plus-test. The only screen with a view toggle; every card defined on both sides. |
| 3 | Financial | **Expense** | ✅ **Closed** — v3, uplifted to the template 2026-08-08 | 3 superseded docs + dev sheet | [[DA-04 Expense — Handoff Sheet]] + both cover notes. Contracts, tap matrix, zeros router, Test it: lines, anchors and collapsed sections all in. No forward setting: Custom stops at today, measured. |
| 4 | Inventory | **Inventory** | ✅ **v2** · uplifted to template 2026-08-08 | 5 superseded docs + no dev-sheet tab | [[DA-08 Inventory — Handoff Sheet]] + both cover notes. Rewritten 12,396 → 9,600 words with every layer applied; decision ledger ran against the rewrite (4 drops, 6 weakenings caught and restored, plus one regression to a defect an audit round had already fixed); design verification found the file unmoved except a staged redesign of Agreements ending soon carrying a Notify Tenant button the sheet now rules out; sibling check found 18 findings across five sheets, all applied. |
...
| 8 | Complaints | **Complaints** | ✅ **Closed, v1.1, all three audits run** | A 62-requirement PRD (Jan 2026, found late, filed off-convention) + dev sheet tab + the homescreen's IO-01 spec | [[DA-10 Complaints — Handoff Sheet]]. Greenfield with no drawn states, no design sign-off, and every number defined from scratch across one owner session, card by card. 79 owner rulings ledger-verified. Biggest single finding: most numbers cannot open their own records today; ten list filters ship with the screen by owner decision. The Complaints empty-state copy leak traced to its Figma origin: five Expense cards duplicated from one Complaints card, layer names never renamed. **The three audits ran the same session** (design verification, source sweep, sibling check) and between them changed the sheet in 40 places, including two definitional holes the author's own ledger could not see: the donut never stated its population, and a past window's donut had no stability rule. Two cover notes written after the audits, not before. |
...
- **Plain punctuation, no em dashes** (locked 2026-08-07). Commas, colons, full stops. The dash survives only as the empty-table-cell placeholder and in document titles. No AI-tell vocabulary. Dues, Collection and Tenants are swept; Expense and Inventory pick it up at their uplift.
...
- **One grammar across the suite** (locked 2026-08-07). Same spine and section names, same table schemas with identical column headers (tap matrix: `You tap | What opens | Arriving filtered to | Ready?` · definitions and View all: `Row | Meaning` · number kinds: `Live` / `Time-scoped`, never a new coinage), same fixed phrases ("Test it:", "as of today", "from today onwards", "hides at zero", the Restricted copy). Tables wherever content is parallel. A sheet may drop what does not apply; it never renames or reorders what it keeps. **Two names settled 2026-08-07:** the access section is **"Who can see this"** everywhere, and the states section is **"What each card shows when it is empty, healthy or broken"** (all five closed sheets now carry them). The filter-behaviour grid is titled **"What every number does on every filter setting"** on every sheet. Collection's old "What is already broken in the code" section is gone as of v13 and is never copied forward.
- **The reader test** (owner-stated 2026-08-08, the acceptance bar over every writing rule): a developer or designer reads the sheet alone and builds from it without asking the PM. Words that fail it get replaced even when technically correct; "partition" failed it on Dues because in a PG a partition is a plywood wall.
- **One time vocabulary, cell level included** (locked 2026-08-08, owner-directed after Dues drifted). The grammar locked section names and table headers but never the words inside cells, so every new layer became fresh surface for coinages; "Follows", "pinned window" and "duration-scoped" all slipped through a grammar that already said "never a new coinage". The fix is a closed list plus a mechanical sweep, not better recall. **Allowed grid-cell phrases:** "As of today" · "Counted inside the window" · "Each tile keeps its own fixed window; the filter does not change it" · "From today onwards" · "—". **Allowed kind labels:** Live · Time-scoped (", window fixed" where pinned) · Forecast. **Banned, and grep-swept at every close-out:** follows, pinned, window-scoped, duration-scoped, period-scoped, any new kind word. Write cells the way you would say them aloud to someone new. Dues carries the corrected wording; every other sheet picks it up at its next touch, and the sweep runs on every sheet from now on.
- **GitHub readability** (locked 2026-08-08, from Dues v11): every sheet's TOC links each section name to its own heading anchor. The tap matrix, the design-fix list, and the measured-figures appendix each collapse behind a GitHub `<details><summary>` block, since each is read start-to-end by one audience only. Section 3's simple one-line terms (no rule, no exclusion, no cross-reference) move into a quick-terms table at the top of the section; anything carrying a real rule keeps its own heading and prose below the table, never folded in just to shorten the section. **Owner-decided as a suite rule after seeing it work on Dues, not before.** **Backfill queue:** Dues and Collection carry it. Expense, Inventory and Tenants apply it at each sheet's next real touch, not as a standalone formatting pass.
- **Every sheet carries a tap matrix** (locked 2026-08-07, same rank as the time-filter grid). One row per tappable element, grouped by card: what opens, the filters already applied on arrival in plain words, and a Ready? verdict. The destination varies with the record's state (arrivals open the Bookings list, departures the Old Tenants list), so "opens a list" without naming which one and how filtered is not a spec. Tenants, Dues and Collection carry it; Expense and Inventory gain the arrival-filter column at their uplift.
- **What travels with a tap depends on the window** (locked 2026-08-07, part of the tap-matrix rule). Live taps carry nothing: the list opens as-of-today. Period taps carry the window, and the list shows those people as they are today, naming any difference on arrival ("showing 11 of the 14; the rest have since moved out"), because a count of events and a list of current people drift apart on any window not ending today. Forward-window taps re-route to where the people are today: scheduled arrivals open the Bookings list, scheduled departures the Tenants list. Records-add-back holds absolutely only on windows that include today.
- **Five behaviour contracts, locked suite-wide 2026-08-07** (found by walking the seams after the tap-matrix work; each is one to three lines on a sheet). **Custom stops at today**: the future belongs to the forward setting, one question never has two answer models. **The filter stays where the manager put it while the app is open**; a fresh launch opens on the default. **Coming back from a drill returns to the screen as it was left, with fresh numbers** (close a record, return, the tile has moved). **On Custom, chips compare the same number of days immediately before the range.** **A day runs midnight to midnight, India time.** All five predate nothing: no sheet had ruled any of them, and every one was a question a builder would have had to ask.
- **Every screen whose records carry future dates gets the forward setting** (locked 2026-08-08, from the Dues backfill). The suite already required *"Custom stops at today, the future belongs to the forward setting"*, and Dues was the one screen with no forward setting to hand the future to: 8,703 bills worth ₹28.5 crore are already raised with due dates more than a week out, and no period setting could reach them. **Coming up** is the name on every screen. Two things differ by screen and must be said per sheet: whether the forward numbers are projections (Tenants, Inventory: state at a future date, confirmed only, no guesses) or plain records (Dues: bills already raised with a future due date, facts not estimates), and which cards sit the setting out because their question is about the past (on Dues, the billed-versus-collected pair). **Trap, verified in the shared date control:** the presets labelled "Next 7 days" and "Next 30 days" resolve backward, to the *last* 7 and 30 days. Any sheet specifying a forward option carries the outcome-plus-test for it.

**The threshold is by-design, not exists-at-all (carved out on Expense 2026-08-08).** A screen gets Coming up when its records carry future dates *by design*, the way bills do. Stray future dates do not qualify: Expense holds 45 future-dated rows out of 340,524, data-entry artifacts, and gets no forward setting; its Custom stops at today and a stray joins totals when its date arrives.
- **Every sheet routes its zeros** (locked 2026-08-07, extending Inventory's healthy-state discovery). One table tells the empty-looking situations apart on sight: never-set-up, **onboarding** (bookings confirmed and nobody moved in: real zeros plus one quiet line, not the setup screen), **the emptied property** (a true and alarming zero, never dressed as setup), the in-window zero with its neutral chip, the good-news zero with healthy wording, the not-recorded gap with its coverage line, and failure. Distinguishing good zero from bad zero from no-data-yet is per number, not per screen.
- **Drill reachability is read from the apps' shared filter drawer, not from the backend** (locked 2026-08-07). Both manager apps use one filter set; that file is the truth for what a manager can reach. On Tenants a late app-side check flipped nine tap-matrix verdicts in both directions: eviction pending/approved, leaving-date windows with a custom range, and the already-expired agreement window all exist as filters, while true disjoint bands (8 to 15 days) and valid-beyond-90 do not. Corollary for the cards: **the lists filter by within-N windows, the cards draw disjoint bands** — a band drill opens the nearest window and says so until band filters exist.
...
Several screens carry a **view toggle** separate from the time filter (Collection: Paid Date / Due Date. Inventory: Bed / Unit. Others likely).
...
### How many rows before Others is decided per screen, from how the categories spread (locked 2026-08-18, from Complaints)

The suite had six numbered rollup cards using three, one using four, five stating no number at all, and only one measured reason anywhere (Collection: median two receivers per property, so three plus Others fits). Complaints measured its own spread and landed on **four**: complaints spread wider across categories than money does across bill types, so three would have left Others holding half of everything, and four named rows cover about three fifths. The rule that explains every existing sheet rather than forcing four of them to change: **the count is set from the measured spread on that screen and stated on the sheet with the figure behind it.** A fixed, closed set of values shows every one and never rolls up (Expense's payment modes). Others is always two steps. Dues' unnumbered category charts and Tenants' two unnumbered tabs should each pick and state their number at next touch.

### A screen may add a filter option its numbers call for; it never removes one of the five (owner, 2026-08-18, from Complaints)

Complaints keeps Last 7 days beside the suite's five, because complaints move in days where money moves in months and half are resolved inside two. Inventory added Now, Tenants dropped it, so the precedent already existed; this makes it a rule. The sheet says why in section 4, and the shared date control does not gain the option on any other screen.

### Naming a sibling screen is allowed in two places only (owner, 2026-08-18, from Complaints)

The no-cross-document rule stands everywhere except section 3's Words to be careful with, where "on Dues a bill is raised to a tenant" is the fact itself, and the design-fix list, where "copied from Tenants" is the fact itself. Collection and Expense already did this and the sibling check flagged it as inconsistent; this settles it for all six. Elsewhere, state the fact and not the sheet.

### An empty state that means the feature is not in use gets the obvious next step; one that means success gets nothing (clarified 2026-08-18, from Complaints)

Inventory's healthy states carry no button because there is nothing to do: a full house is success. Its whole-screen not-set-up state carries one ("Let's set up my property") because setup is the next step. Complaints made the distinction explicit: a screen with no complaints almost never means perfection, it means the feature is not in use, so the whole-screen state carries **Add Complaint** and reads plainly, never triumphantly. The rule was already Inventory's; it just had not been said in one sentence.
...
- **GitHub mirror:** `https://github.com/eazyapp-tech/manager-app-detailed-analytics` (private, local clone `~/manager-app-detailed-analytics`). **This vault folder is the source of truth.** The repo carries the closed deliverables only: each screen's handoff sheet plus its cover notes, the tracker, and the template. Briefs, formula maps, archives and superseded sources stay in the vault and never go to the repo (owner's scope, 2026-08-07, after a full mirror was pushed and rejected as overwhelming for readers). Filenames match the vault exactly, so sync = copy the changed files + commit, at every screen close.
...
### Collection backfill — paid 2026-08-08

All eight items applied at v13, same as Dues' 8 August backfill.

### Expense uplift — paid 2026-08-08

All layers applied at v3 in one session: cell vocabulary, contracts, tap matrix with arrival filters and the drawer-grounded capability summary, zeros router, Test it: conversion, anchors and collapsed sections, both cover notes, repo synced. Three v2 rulings were reversed with owner approval: no forward setting with Custom stopping at today (45 of 340,524 expenses future-dated), a neutral chip on the count tile, and the Passbook drill carrying nothing because a balance has no window.

### Inventory uplift — paid 2026-08-08

All layers applied at v2 in one session: template spine kept (v1's numbering already matched, so every README deep link survived), cell vocabulary including a **new closed phrase added to the template** ("Averaged across the days of the window", for state numbers; Complaints will need it), contracts, tap matrix with arrival filters, zeros router, Test it: conversion, anchors and collapses, both cover notes, 189 em dashes swept to sanctioned-only. Owner decisions during the uplift: the red-largest-aging-bar stays as a named per-screen exception to the red rule; the trend keeps plotting the level as the suite's one exception (movement belongs to Tenants); chips-on-Now stays (state numbers have predecessors; the Tenants chip line is scoped to event screens); "Averaged across the days of the window" added to the closed list. The queue is now empty.
...
|---|---|---|---|
| 2026-08-06 → 2026-08-08 | `37fb37a0-57ee-4862-ad27-68d4335b8c5a` | Dues | Closed at v7 on 08-06 (Figma pass → owner review → code verification → `grilling` on 2 final threads). Method extracted into the `screen-handoff-pipeline` skill (pushed to `eazyapp-tech/screen-handoff-pipeline`) and this tracker. Most cross-screen decisions above came from this session. **Same session, resumed 08-08 for the Dues backfill to v11**: restructured onto the template, production-sized (99.5% of unpaid money already overdue; the 22+ ageing bucket alone is 92% of it, split into 22–90/90+), the tap matrix rebuilt against the web app's Money filter drawer (a cross-tab drill precedent set, Received figures open the Collection list), a design-file verification and source-document sweep run as sub-agents (one real self-contradiction caught, the Bills Summary equality claim), and a sibling check against Tenants that found and fixed a real gap Tenants had carried since it closed: no share-of-total on its own property-wise card, now retrofitted with a what-changed note at the top of that sheet. Two cover notes and three superseded banners written. **Closed at v12 after four owner-driven rounds past "done":** the forward setting (Dues was the one screen with none, while the suite already required Custom to stop at today and hand the future to one; 8,703 bills worth ₹28.5 crore sat unreachable), GitHub navigation (TOC anchors, collapsed reference sections), one time vocabulary down to cell level (the grammar had locked headers but never cell text, so each new layer regrew jargon), and the reader test applied word by word (which killed "partition", correct English that means a plywood wall in a PG). The last check before close found what the greps could not: removing one banned word had left five different phrasings for one idea, and the cover notes had never been swept at all. |
...
| 2026-08-08 | `f164d8c6-2215-48be-bcab-f30719bbb0a1` | Collection backfill | Closed at v13. The v12 content restructured onto the template with nothing re-derived: a 98-probe decision ledger ran against the new text and every ruling survived. Production sizing (18 measurements) killed nothing and corrected two claims: caution-money adjustments are live since April, not "unbuilt", and RentOk-credit payments are near-dead (27 ever). The filter-drawer enumeration settled every tap-matrix verdict and found four traps: widget-code arrivals wipe every drawer filter, the drawer's cash option reaches one of two cash codes, the deposit-adjusted widget on the live strip fails when tapped, and a six-state settlement filter sits fully built but never enabled in the mobile app. Owner decisions: **Coming up arrives, Due Date view only, Custom stops at today** (the future-Custom rule died with the straddle case); the cut order was settled and then revised when the owner protected the View all sheet as a suite pattern; **no new list screens**, after the owner caught a settlements-list recommendation that ignored the existing FlexiPe screen (trap 16's shape, second occurrence) and redirected the whole gap list to filters on existing lists; bank-destination rows draw from the older settlement system's records too, since the newer one covers 160 properties ever. Biggest open item shipped to engineering: a third of July's online money (₹48 crore) has no settlement record in any system, and the Unsettled tile cannot ship until no-record is confirmed to mean not-settled. Two cover notes written; repo synced. **Post-close, both remaining audit angles ran on owner request and earned their keep:** the source sweep against v12 (recovered from the repo's git history) found 12 real drops from the full rewrite plus two overclaims, and the design verification confirmed 21 of 25 fix claims while catching one stale one: the property rows are now drawn sorted, because **design is already applying v12's fixes and the file moved under the audit**. One agent finding contradicted two of my own earlier frame reads; a fresh render proved the agent right and the file changed, not the reading. The three superseded banners gained their own corrections, and the sheet's card sections were reordered to match the drawn screen (Trend ninth, By Property tenth). |

| 2026-08-08 | `8777dbce-be50-4586-b3b7-6e0a99e17c99` | Expense uplift | Closed at v3. The prompt's grounded debt list survived contact perfectly; my own first verification round did not — three measurement errors in one pass (a case-sensitive grep missing 31 capitalised "Follows", the wrong fixed string for the Test convention, the wrong repo path) each read as "the prompt is stale" until re-measured. Two debts the prompt missed were found in grounding: the Nimit cover note's reading map had gone stale against a renumbered spine and was telling the backend reader to skip Build guidance, and one live prose em dash from the v2 session survived the corpus sweep. The forward setting was settled from production, not copied from either sibling: 45 of 340,524 expenses are future-dated, so no Coming up, Custom stops at today, and the suite rule gained its by-design threshold. Three v2 rulings reversed with owner approval (forward setting, neutral count chip, Passbook drill carries nothing, which also deleted the open engineering check). The two audit angles ran as sub-agents: the source sweep found 7 real drops, 8 contradictions including a v3-internal one its own patches had created, and 5 grammar hits, with 3 findings rejected against the exemplars; the design verification held 40 of 41 claims, found the file unmoved since 7 August, and surfaced 5 unmentioned facts including the two tile-row copies disagreeing on their fourth tile. Sibling check found the suite's Test convention three-way split and two missing contracts on Dues and Tenants. Inventory's uplift was measured and is a full session: 12,396 words, 189 em dashes, no layers. Repo synced at 7f4c407 with both cover notes and the README row. |
| 2026-08-08 | `de9381a9-caf3-4e21-b534-04069a0fcfd4` (same session as v1) | Inventory uplift | Closed at v2. The prompt's measured debt list held exactly (12,396 words · 189 em dashes · 13 banned time words · 4 tests · no layers); one figure off by 36 words (Dues 6,248, not 6,284). Grill before drafting settled four collisions with post-v1 suite rules: a **new closed cell phrase** ("Averaged across the days of the window") added to the template for state screens; red-largest-aging-bar kept as a named per-screen exception; the trend keeps the level as the suite's one exception; chips-on-Now kept with the Tenants line scoped to event screens. The rewrite landed at 9,600 words, a hair over the Collection ceiling after the audits restored dropped rulings; the spine is unchanged, so every README deep link survived. **The decision ledger earned its keep**: 4 dropped rulings, 6 weakenings, and one true regression — the rewrite had reintroduced the forward-cards-narrow-on-Coming-up behaviour that a v1 audit round had explicitly removed. **Design verification caught the file moving again**: a staged redesign of Agreements ending soon (bars, "No. of lease" still, a typo, and a **Notify Tenant** button that breaks the not-a-management-screen rule; the sheet now routes that action to the tenants list). **The sibling check produced its biggest haul yet, 18 findings across all five sheets**: Inventory described the Under-Eviction disagreement Collection had already settled; Tenants asserted a booked-count equality that production breaks by 931 bookings; Dues' status bar contradicted its own confirmed-only rule; two siblings still carried banned time words; and Expense's toggle line had already fixed itself, closing Inventory's open item 10 without an edit. All applied with what-changed notes. One process scar: an argument-evaluation bug in a fix script truncated Collection's sheet to zero bytes; restored from the repo clone and verified against the sibling check's own quotes — the repo mirror just paid for itself as a backup. |
| 2026-08-09 → 2026-08-10 | `844bb8f1-c96c-4528-9ed3-41366e3b2525` | Dues (View all re-audit) | Owner reported the View all sheet looked thinner than the rest of the doc. It was: §6 had been written from a frame the doc dismissed as an old draft, and the frame turned out to be current and complete, close enough to the doc's own vocabulary to document almost as-is. Rewrote §6 as a Row \| Meaning table from it; picked up one live bug the dismissal had let past unaudited (a change chip on Past Dues, a number the screen's own rule says carries none) and three smaller ones (a rent-only mislabel on the Projected row, a misfiled bottomsheet from an unrelated screen, a duplicate hidden chip layer per tile); corrected one design-fix item that had already been fixed in Figma. Swept the other four closed sheets for the same dismissal shape and found none — Collection's "not drawn yet" for its own View all is an honest absence, everything else was already-logged design debt. Generalized into a new locked rule: any claim that a frame is stale, superseded or not drawn gets a live Figma re-check before the sheet is next touched, never a recall; still owed on Old Tenants, Bookings, Complaints and Leads once each has a design file. Repo synced at `4a1293e` and `e8d467b`. |
| 2026-08-17 → 2026-08-18 | `9c5d7a87-11d2-4b8a-abd5-db4aad59ff15` (continued from `b4391da8-9ec4-4b65-9f7b-d86d63b724d4`, `db63ed2f-642d-4a44-93f7-18f6fa737e77`, `9f097f24-f726-4f6d-8e50-5b95d7ecadda`) | Complaints | Closed at v1. The greenfield screen with the least going in and the most found. Phase 0 surfaced a 62-requirement PRD filed off-convention (the Tenants trap, second time), the homescreen's own active-in-period spec, and that the backend stub emits no copy at all, so the leaked empty-state string was never backend-sourced. Production sizing ran four passes and reshaped the screen: 38,365 open complaints with a median age of 444 days against a resolve time of two days (two populations wearing one word), only seven status codes in use out of ten claimed, no Rejected ever written, 63% of stored ratings being the value 6 (raw average 4.99, real 3.3), a 5.5-hour clock mismatch between complaints and their history, and escalation firing emails and recording nothing. **The owner ran the whole session card by card, one question at a time**, after the first consolidated report was rejected as overwhelming; that cadence held for eleven cards, the View all sheet, filter, chips, insights, permissions, states, tap matrix, non-goals and info icons. Owner overrules that changed the sheet: TAT drives overdue (not a flat clock), keep the true average (not the median), Open not Unresolved, all-complaints donut (not open-only), red on late ageing bars, admin separated from team, four categories not three, filters ship with the screen. Two rules I proposed were reversed by data mid-session (per-card ageing dropdown, backlog stale-layer) and one by the owner's own reasoning (Open Backlog scoped by the filter). The sheet's info-icon copy is the suite's first, written to the shipped homescreen voice after that voice was measured (46 hints, median 7 words, no you/your, four calc blocks). Traps that fired: a size-window grep missed 'follow' by sweeping only 'follows'; a rule inherited from recall (Inventory's no-button empty state) was wrong on re-read. Repo sync at close. |

---

## Next action

**Old Tenants (screen 6)**, under People. Complaints closed 2026-08-18; Old Tenants and Bookings resume their place in the order, then Leads.

Known going in:

- **No developer-sheet tab and no design states.** The workbook holds only Issues, Tenants, Dues, Collection, Expense. Old Tenants is the first screen with no informal source of any kind; the design file and production carry the whole grounding load, as they did on Tenants.
- **A cancelled booking lands on the Old Tenants list** even though the person never lived there (owner ruling on Inventory). The screen must separate real departures from cancelled bookings or its headline overstates departures. Settled principle, unbuilt distinction.
- **Moved-out tenants' dues are deliberately excluded from Dues** and have one home, here. Inherit the exact wording from Dues section 3.
- **The active-in-period model now has three consumers**: the homescreen Issues card (ticketed), Complaints Open Backlog (specified), and almost certainly a departed-in-period count here. Build once.
- **Complaints spawned two suite-wide checks worth running here**: sweep 'follow' in every inflection, not just 'follows'; and re-read any inherited rule against its source sheet before writing it as a fact (an Inventory no-button rule was recalled wrong).
- Complaints' owner cadence, one card and one question at a time, worked; the first consolidated report did not. Start card by card.

After Old Tenants: **Bookings** (inherits the booking definitions, including booked-is-a-layer and the no-space row reserved on Inventory's View all sheet).
